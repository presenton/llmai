...
from llmai import AsyncOpenRouterClient, OpenRouterClient, OpenRouterClientConfig
...
def make_async_client() -> AsyncOpenRouterClient:
...
def test_generate_tool_loop():
    run_tool_loop(make_client(), model=MODEL, label="OpenRouter")
...
async def test_agenerate():
    await _agenerate(
        "OpenRouter async plain generation",
        messages=[
            UserMessage(content="What is presentation?"),
        ],
    )
...
async def test_agenerate_structured():
    await _agenerate(
        "OpenRouter async structured generation",
        messages=[
            UserMessage(content="What is presentation?"),
        ],
        response_format=make_response_format(),
...
async def test_agenerate_structured_strict():
    await _agenerate(
        "OpenRouter async strict structured generation",
        messages=[
            UserMessage(content="What is presentation?"),
        ],
        response_format=make_response_format(strict=True),
...
async def test_agenerate_tool_calls():
    await _agenerate(
        "OpenRouter async tool-call generation",
        messages=[
            UserMessage(content="What is presentation?"),
        ],
        tools=TOOL_DEFINITIONS,
...
async def test_agenerate_tool_loop():
    async with make_async_client() as client:
        await arun_tool_loop(client, model=MODEL, label="OpenRouter async")


async def test_agenerate_web_search():
    await _agenerate(
        "OpenRouter async web-search generation (ignored by provider adapter)",
        messages=[
            UserMessage(
                content="What was a positive news story from today? Cite sources."
            ),
...
async def test_astream_tool_calls():
    await _astream(
        "OpenRouter async tool-call stream",
        messages=[
            UserMessage(content="What is presentation?"),
        ],
        tools=TOOL_DEFINITIONS,
